...
* fast  - updates `[tool.zephyrus.coverage_fast].fail_under` (unit/smoke gate)
...
def read_threshold_from_pyproject(target: str) -> float:
    ...
    try:
        ...
        if target == 'fast':
            return float(data['tool']['zephyrus']['coverage_fast']['fail_under'])
...
def update_threshold_in_pyproject(target: str, new_threshold: float) -> bool:
    ...
    if target == 'full':
        ...
        zephyrus_section = document.get('tool', {}).get('zephyrus')
        if zephyrus_section is None:
            raise ValueError('[tool.zephyrus] section not found in pyproject.toml')
        coverage_fast = zephyrus_section.get('coverage_fast')
        if coverage_fast is None:
            raise ValueError(
                '[tool.zephyrus.coverage_fast] section not found in pyproject.toml'
            )
