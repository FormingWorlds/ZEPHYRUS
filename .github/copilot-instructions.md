...
- PROTEUS passes `scaling=3`; the pinned escape tests pass `scaling` explicitly, so no test guards the default. A change of the default updates the `EL_escape` docstring and every docs page that names it, and adds a test that pins it.
...
This checklist repeats review points from `AGENTS.md` (its Review section and its physics rules) and `tests/AGENTS.md`; change them together.
