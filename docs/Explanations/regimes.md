...
All knobs, their defaults, and their meanings are tabulated in the [parameter reference](../Reference/parameters.md); the defaults are the documented reference choices used throughout this page. The assumptions that remain on every result, whatever the knobs, are collected on the [limitations page](limitations.md).
