...
| `nozzle_periapsis` | `True` | The nozzle won on an eccentric orbit. The geometry was evaluated at periapsis, matching the screen's periapsis Hill radius, on a model built for a circular synchronous donor; elsewhere on the orbit the instantaneous rate is lower. | The rate reflects it |
| `near_roche` | `True` | The Hill radius is less than 1.5 flow radii, that is, the flow reaches beyond two thirds of the way to the lobe. The tidal factor is steep there; read `diagnostics['roche']['tidal_inflation']` for how much of the rate it is setting. | Reporting only |
