...
for i,t in enumerate(time):
    ...
    mass_loss_H[i] = Planetary_surface_area(Rp[i]) * Phi_l * delta_t / s2yr # Mass loss of H [kg/s]
    mass_loss_O[i] = Planetary_surface_area(Rp[i]) * Phi_h *delta_t  / s2yr# Mass loss of O [kg/s]
    mass_loss_total[i] = Planetary_surface_area(Rp[i]) * Phi_EUV_Collin_i *delta_t / s2yr # Total mass loss [kg/s]
    ...
    Phi_critical[i] = Phi_c 
    Phi_light[i] = Phi_l * mu_H
    Phi_heavy[i] = Phi_h * mu_O
    Phi_total[i] = Phi_tot * (x_light[i]*mu_H + x_heavy[i]*mu_O)
    Phi_EUV_Collin[i] = Phi_EUV_Collin_i
    Phi_EL = Phi_EL_i# *delta_t / s2yr
...
plt.figure(figsize=(10, 6))
plt.plot(time, mass_loss_H*mu_H/Me, label='H loss Zephyrus', color='dodgerblue')
plt.plot(time, mass_loss_O*mu_O/Me, label='O loss Zephyrus', color='red')
...
axs[2].plot(time, mass_loss_H*mu_H/Me, label='H loss Zephyrus', color='dodgerblue', linestyle=':', linewidth=2)
axs[2].plot(time, mass_loss_O*mu_O/Me, label='O loss Zephyrus', color='red', linestyle=':', linewidth=2)
axs[2].plot(time, mass_loss_total/Me, label='Total loss Zephyrus', color='purple', linestyle=':', linewidth=2)
axs[2].plot(time, escape_EL, label='EL loss PROTEUS', color='pink', linestyle='-')
