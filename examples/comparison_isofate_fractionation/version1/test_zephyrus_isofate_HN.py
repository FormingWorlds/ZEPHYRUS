...
sol_isofate = isocalc(f_atm, Mp, Mstar, F0, Fp, T, d, time = 5e9, mechanism = 'XUV', species = 'H/N', rad_evol = True,
...
sol_zephyrus = isocalc_zephyrus(f_atm, Mp, Mstar, F0, Fp, T, d, time = 5e9, mechanism = 'XUV', species = 'H/N', rad_evol = True,
mu = mu_solar, eps = 0.15, activity = 'medium', flux_model = 'power law', stellar_type = 'G',
Rp_override = False, t_sat = 50e6, f_atm_final = 'null', n_TO_final = 'null', 
n_steps = int(1e5), t0 = 1e6, rho_rcb = 1.0, Johnson = False, RR = True, f_pred = False, thermal = True, 
beta = -1.23)
...
axs[0].plot(t_a_isofate*s2yr, Phi2_a_isofate[0]*mu_N, linestyle='-', label='N flux', color='deeppink')
cross_isofate = np.where(np.abs(phi_a_isofate[0] - phic_a_isofate[0]) < 1e-15)
if len(cross_isofate[0]) != 0:
...
axs[0].plot(t_a_zephyrus*s2yr, Phi2_a_zephyrus[0]*mu_N, linestyle=':',color='deeppink', linewidth=2)
cross_zephyrus = np.where(np.abs(phi_a_zephyrus[0] - phic_a_zephyrus[0]) < 1e-15)
if len(cross_zephyrus[0]) != 0:
...
axs[1].plot(t_a_isofate*s2yr, N2_a_isofate[0]/avogadro, linestyle='-', label=r'$N_N$ IsoFATE', color='deeppink')
axs[1].plot(t_a_zephyrus*s2yr, N1_a_zephyrus[0]/avogadro, linestyle=':', label=r'$N_H$ Zephyrus', color='dodgerblue', linewidth=2)
axs[1].plot(t_a_zephyrus*s2yr, N2_a_zephyrus[0]/avogadro, linestyle=':', label=r'$N_N$ Zephyrus', color='deeppink', linewidth=2)
if len(cross_isofate[0]) != 0:
...
axs[3].plot(t_a_isofate*s2yr, x2_a_isofate[0]*100, linestyle='-', label=r'$x_N$ IsoFATE', color='deeppink')          
axs[3].plot(t_a_zephyrus*s2yr, x1_a_zephyrus[0]*100, linestyle=':', label=r'$x_H$ Zephyrus', color='dodgerblue', linewidth=2)
axs[3].plot(t_a_zephyrus*s2yr, x2_a_zephyrus[0]*100, linestyle=':', label=r'$x_N$ Zephyrus', color='deeppink', linewidth=2)
...
axs[2].plot(t_a_isofate*s2yr, (N2_a_isofate[0]/N1_a_isofate[0])/NtoH_protosolar, linestyle='-', label=r'IsoFATE', color='maroon')
axs[2].plot(t_a_zephyrus*s2yr, (N2_a_zephyrus[0]/N1_a_zephyrus[0])/NtoH_protosolar, linestyle=':', label=r'Zephyrus', color='maroon', linewidth=2)
axs[2].set_xscale('log')
# axs[2].set_yscale('log')
axs[2].set_xlabel('Time [yr]', fontsize=14)
axs[2].set_ylabel(r'N/H [Solar]', fontsize=14)
axs[2].legend()
...
axs[7].plot(t_a_isofate*s2yr, mlossh_a_isofate[0]/Me, linestyle='-', label='N loss IsoFATE', color='deeppink')
axs[7].plot(t_a_zephyrus*s2yr, mlossl_a_zephyrus[0]/Me, linestyle=':', label='H loss Zephyrus', color='dodgerblue', linewidth=2)
axs[7].plot(t_a_zephyrus*s2yr, mlossh_a_zephyrus[0]/Me, linestyle=':', label='N loss Zephyrus', color='deeppink', linewidth=2)
...
fig.suptitle('Comparison fractionation IsoFATE vs Zephyrus : Binary mixture H-N', fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.96])

plt.savefig('plots_HN/8panels_comparison_zephyrus_HN.png', dpi=300)
plt.close()
