...
axs[0].plot(t_a_zephyrus*s2yr, Phi2_a_zephyrus[0]*mu_O, linestyle=':',color='red', linewidth=2)
cross_zephyrus = np.where(np.abs(phi_a_zephyrus[0] - phic_a_zephyrus[0]) < 1e-15)
if len(cross_zephyrus[0]) != 0:
    axs[0].axvline(t_a_zephyrus[cross_zephyrus][0]*s2yr, ls = '--', lw = 0.5, color = 'coral')
axs[0].set_xscale('log')
axs[0].set_yscale('log')
axs[0].set_ylim(1e-11, 1e-9)
...
axs[2].plot(t_a_isofate*s2yr, (N2_a_isofate[0]/N1_a_isofate[0])/OtoH_protosolar*100, linestyle='-', label=r'IsoFATE', color='maroon')
axs[2].plot(t_a_zephyrus*s2yr, (N2_a_zephyrus[0]/N1_a_zephyrus[0])/OtoH_protosolar*100, linestyle=':', label=r'Zephyrus', color='maroon', linewidth=2)
...
axs[7].plot(t_a_isofate*s2yr, mlossh_a_isofate[0]/Me, linestyle='-', label='O loss IsoFATE', color='red')
axs[7].plot(t_a_zephyrus*s2yr, mlossl_a_zephyrus[0]/Me, linestyle=':', label='H loss Zephyrus', color='dodgerblue', linewidth=2)
axs[7].plot(t_a_zephyrus*s2yr, mlossh_a_zephyrus[0]/Me, linestyle=':', label='O loss Zephyrus', color='red', linewidth=2)
axs[7].set_xscale('log')
axs[7].set_yscale('log')
axs[7].set_ylim(1e-10, 1e-6)
...
fig.suptitle('Comparison fractionation IsoFATE vs Zephyrus : Binary mixture H-O', fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.96])

plt.savefig('plots_HO/8panels_comparison_zephyrus_HO.png', dpi=300)
plt.close()

plt.style.use('tableau-colorblind10')
# Figure 10: Mass loss per timestep
plt.figure(figsize=(10, 6))
plt.plot(t_a_isofate*s2yr, mloss_a_isofate[0]/Me, linestyle='-', label='IsoFATE')
plt.plot(t_a_zephyrus*s2yr, mloss_a_zephyrus[0]/Me, linestyle=':', label='Zephyrus', linewidth=2)
plt.plot(t_a_isofate*s2yr, mlossl_a_isofate[0]/Me, linestyle='-', label='H loss IsoFATE')
plt.plot(t_a_isofate*s2yr, mlossh_a_isofate[0]/Me, linestyle='-', label='O loss IsoFATE')
plt.plot(t_a_zephyrus*s2yr, mlossl_a_zephyrus[0]/Me, linestyle=':', label='H loss Zephyrus', linewidth=2)
plt.plot(t_a_zephyrus*s2yr, mlossh_a_zephyrus[0]/Me, linestyle=':', label='O loss Zephyrus', linewidth=2)       
plt.xscale('log')
plt.yscale('log')
plt.ylim(   1e-10, 1e-6)
plt.xlabel('Time [yr]', fontsize=16)
