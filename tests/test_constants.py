...
def test_gravitational_constant_si_cgs_consistency():
    ...
    """
    assert G == pytest.approx(6.6743e-11, rel=1e-12, abs=0.0)
    # cgs/SI consistency: the ratio must be exactly 1e3.
    assert G_cgs / G == pytest.approx(1e3, rel=1e-9, abs=0.0)
...
def test_speed_of_light_is_the_exact_si_value():
    ...
    """
    assert c == pytest.approx(299792458.0, rel=1e-12, abs=0.0)
...
def test_unit_conversions_are_mutually_consistent():
    ...
    """
    assert erg2joule == pytest.approx(1e-7, rel=1e-12, abs=0.0)
    # Length: centimetres are exactly 1e2 metres.
    assert au2cm / au2m == pytest.approx(1e2, rel=1e-9, abs=0.0)
    # Energy flux: erg s-1 cm-2 -> W m-2 folds erg->J (1e-7) with cm^-2->m^-2 (1e4).
    assert ergcm2stoWm2 == pytest.approx(erg2joule * 1e4, rel=1e-9, abs=0.0)
    # Power: erg s-1 -> W is the plain erg->J factor.
    assert ergpersecondtowatt == pytest.approx(erg2joule, rel=1e-12, abs=0.0)
...
def test_seconds_to_year_uses_the_365_day_convention():
    ...
    """
    assert s2yr == pytest.approx(1.0 / (3600 * 24 * 365), rel=1e-12, abs=0.0)
    julian = 1.0 / (3600 * 24 * 365.25)
    # The 365-day and Julian conventions are resolvably different.
    assert s2yr != pytest.approx(julian, rel=1e-4, abs=0.0)
...
def test_boltzmann_constant_is_the_exact_si_value():
    ...
    """
    assert kb == pytest.approx(1.380649e-23, rel=1e-12, abs=0.0)
    # cgs/SI consistency: J -> erg is exactly 1e7.
    assert kb_cgs / kb == pytest.approx(1e7, rel=1e-12, abs=0.0)
...
def test_planck_constant_and_electronvolt_are_exact_si_values():
    ...
    """
    assert h_planck == pytest.approx(6.62607015e-34, rel=1e-12, abs=0.0)
    assert ev2joule == pytest.approx(1.602176634e-19, rel=1e-12, abs=0.0)
...
def test_proton_and_atomic_mass_constants_are_consistent():
    ...
    """
    assert m_p == pytest.approx(1.67262192369e-27, rel=1e-12, abs=0.0)
    assert amu == pytest.approx(1.66053906660e-27, rel=1e-12, abs=0.0)
    # The proton is 0.73% heavier than one atomic mass unit.
    assert m_p / amu == pytest.approx(1.007276467, rel=1e-6, abs=0.0)
